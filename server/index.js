...
let gameState = {
  deck: [],
  dealerHand: [],
  players: new Map(), // Map<playerId, {id, name, hand, score, status, result}>
  gameStatus: 'waiting', // 'waiting' | 'dealing' | 'playing' | 'dealer-turn' | 'finished'
  currentPlayerIndex: 0,
  maxPlayers: 3,
};

// Import game utilities (we'll need to create a shared version or duplicate logic)
...
function createMultipleDecks(numDecks = 6) {
  const deck = [];
...
function nextPlayer() {
  ...
  // All players are finished, move to dealer
  gameState.currentPlayerIndex = 0;
...
io.on('connection', (socket) => {
  ...
  socket.on('dealCards', () => {
    ...
    if (allBlackjack) {
      ...
      gameState.gameStatus = 'playing';
      // Find first player who is not finished (doesn't have blackjack)
  ...
  socket.on('disconnect', () => {
    ...
    // If game is in progress, end it for all players
    if (gameInProgress && gameState.players.size > 0) {
      ...
      players.forEach((player) => {
        player.status = 'finished';
        // Don't overwrite existing results (win/lose/etc), but set to null if they were still playing
        if (!player.result) {
          player.result = null;
        }
      });
      ...
      // Broadcast with disconnect message
      ...
      // Game was waiting, just adjust player index if needed
      const players = getPlayerArray();
      if (gameState.currentPlayerIndex >= players.length) {
        gameState.currentPlayerIndex = 0;
      }
